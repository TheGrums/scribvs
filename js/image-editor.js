...
ImageManipulator.prototype = {
    ...
    init: function() {
      ...
      window.addEventListener("touchleave", function(event) {
        window.removeEventListener('touchmove', self.boundDrag, false);
      });
      ...
      canvas.addEventListener("touchstart", function(event) {
        ...
        self.mobileDragStart(event);
      });

      $("body").on("change", function(){

        value = ($(".ui-slider-handle").attr("aria-valuenow"))/100;
        self.scale(value);
    ...
    save: function() {
      ...
      /*
        ...
        Modified code to really save the image on the
        server with AJAX and dataURL.
      ...
      */
      ...
      $.ajax({
        ...
        success: function(data) {
          lightboxClose();
          $(".profile-img").css("background-image"," url("+data._img+")");
          $(".pub-img").attr("src",data._img);
        },
        error: function(){
          console.log("error");
        }
    ...
    mobileDragStart: function(event) {
      mousePosition = this.getMousePosition(event);
      ...
      window.addEventListener("touchmove", this.boundDrag, false);
    },
    ...
    getMousePosition: function(event) {
      ...
      if(event.type!="touchmove"&&event.type!="touchstart"){
        ...
        var mousePosition = {
          x: event.touches[0].clientX - canvasElement.left,
          y: event.touches[0].clientY - canvasElement.top
        };
